    ...
    try:
        ...
        try:
            ...
            for target_idx in range(_start_idx, total_records):
                print(f"\n" + "="*50)
                print(f"📦 正在处理记录: 第 {target_idx + 1} 条 (总计 {total_records} 条)")
                print("="*50)
                ...
                print(f"DEBUG - [主循环] 记录 {target_idx + 1} 处理结束，状态检查: {'已就绪' if current_record else '为空(None)'}")
                ...
                if current_record is not None:
                    ...
                    if success:
                        ...
                        # --- 核心修改：上传成功后再清理本地图片 ---
                        if local_paths:
                            print(f"   🗑️  清理记录 {target_idx + 1} 的本地临时图片...")
                            import os
                    ...
                    print(f"   ⚠️  警告: 第 {target_idx + 1} 条记录在 feishu_records 中仍然为空，跳过同步。")
